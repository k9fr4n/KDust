    ...
    return NextResponse.json(
      {
        ...
        message:
          ...
          `/api/task/${run.task.id}/run with an explicit { project } body.`,
      },
      { status: 400 },
    );
  }

  // Best-effort actor attribution (same pattern as /api/task/:id/run).
