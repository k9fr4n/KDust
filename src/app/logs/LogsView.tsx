...
/**
 * Format a Unix-ms timestamp as HH:MM:SS.mmm in the configured
 * AppConfig timezone. Mirrors what `new Date(ts).toISOString()
 * .substring(11, 23)` did, but honours the operator's tz choice
 * instead of always rendering UTC. Uses a memoised
 * Intl.DateTimeFormat for hot-path performance (one render of
 * /logs can churn through a few thousand entries).
 */
function makeTimeFormatter(tz: string) {
  ...
  try {
    ...
    // Defensive fallback: invalid tz should never reach here
    // (validated at /api/settings) but degrade to UTC instead
  ...
  return (ts: number) => {
    // en-GB with hour12:false yields "HH:MM:SS.mmm" directly,
    // matching the previous on-screen format.
    return fmt.format(new Date(ts));
  };
}

export default function LogsView({ tz }: { tz: string }) {
  const formatTime = useMemo(() => makeTimeFormatter(tz), [tz]);
  ...
  const copy = async () => {
    const text = filtered
      .map((e) => `${formatTime(e.ts)} [${e.level}] ${e.text}`)
      .join('\n');
    try {
        ...
        <div className="p-3 whitespace-pre-wrap">
          ...
          {filtered.map((e) => (
            <div key={e.id} className={LEVEL_COLOR[e.level]}>
              <span className="text-slate-500" title={new Date(e.ts).toISOString()}>
                {formatTime(e.ts)}
              </span>{' '}
