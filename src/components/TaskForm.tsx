...
export type CronFormValues = {
  ...
  /**
   * Routing metadata (Franck 2026-04-29, ADR-0002). Surfaced by the
   * task-runner MCP server in list_tasks/describe_task so an
   * orchestrator (or chat assistant) can pick the right task without
   * parsing the full prompt.
   *
   *   description  — 1-3 sentences "what is this task for", written
   *                  for the routing layer. Distinct from `prompt`.
   *   tags         — comma-separated keywords in the UI; serialised
   *                  to a JSON array on the wire.
   *   inputsSchema — raw JSON Schema string; persisted as-is.
   *                  Must parse to valid JSON (server enforces).
   *   sideEffects  — declares what the task does to the workspace.
   *                  'readonly' means the orchestrator can dispatch
   *                  it without confirmation; 'pushes' triggers the
   *                  most cautious gate.
   */
...
export function TaskForm({
  ...
  const [form, setForm] = useState<CronFormValues>({
    ...
    // ADR-0002 routing metadata defaults. Empty / null on create;
    // sideEffects defaults to 'writes' to match the schema default
    // (conservative — orchestrator gates a 'writes' or 'pushes'
    // dispatch behind a confirmation, lets a 'readonly' through).
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        // The API normalises again server-side (strings or array
        // both accepted) — sending an array here is just clearer.
  ...
  return (
    <form onSubmit={submit} className="max-w-6xl mx-auto space-y-4">
      ...
      {/* ---------------------------------------------------------
          Routing metadata (Franck 2026-04-29, ADR-0002)
          ---------------------------------------------------------
          Surfaced by the task-runner MCP server in
          list_tasks / describe_task so an orchestrator (or the
          chat assistant) can pick the right task without parsing
          the prompt — which is written for the executing agent,
          not for the routing layer. All four fields are optional;
          sideEffects defaults to 'writes' (conservative gate). */}
      <fieldset className={sectionCls}>
