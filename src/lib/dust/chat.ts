...
/**
 * Files generated by an MCP tool during a single agent turn, as
 * exposed on `agent_action_success.action.generatedFiles[]` by the
 * Dust SDK (see `AgentActionSuccessEventSchema` in
 * @dust-tt/client/dist/types.d.ts). We keep only the fields we
 * actually surface in the UI; the SDK union of `contentType`
 * literals is wide and contributes no value to our renderer
 * (we mime-route by prefix `image/`).
 */
...
type AgentActionSuccessShape = {
  ...
  action: {
    ...
    /**
     * Optional in this local shape because (a) some Dust payloads in
     * the wild omit it when a tool produced no files, and (b) we
     * defensively narrow it at the call site so a future SDK rename
     * does not crash the stream loop. See SDK schema for the full
     * `contentType` literal union.
     */
...
export interface StreamStats {
   ...
   * arrival, deduped by `fileId` (first occurrence wins), and
   * filtered for `hidden=true` so the UI never advertises files Dust
   * meant to keep internal. May be empty.
   */
  generatedFiles: AgentGeneratedFile[];
...
/**
 * Serialise a `StreamStats.generatedFiles` list for SQLite storage
 * AND for SSE wire payloads. Returns null when the list is empty so
 * DB rows stay NULL. A 50 KB cap mirrors `toolInvocationsToJson()`:
 * even at 256 B per row that fits ~200 files, well beyond any
 * realistic agent turn. Each `snippet` field is independently
 * truncated to 500 chars so a single oversized preview cannot
 * dominate the row.
 *
 * The output shape is intentionally identical to the in-memory
 * `AgentGeneratedFile` array so the UI can JSON.parse() it once and
 * render directly — no normalisation layer required.
 */
export function generatedFilesToJson(
  files: AgentGeneratedFile[] | null | undefined,
): string | null {
  if (!files || files.length === 0) return null;
  const SNIPPET_CAP = 500;
  const TOTAL_CAP = 50_000;
  const compact: AgentGeneratedFile[] = files.map((f) => ({
    fileId: f.fileId,
    title: f.title,
    contentType: f.contentType,
    snippet:
      typeof f.snippet === 'string' && f.snippet.length > SNIPPET_CAP
        ? f.snippet.slice(0, SNIPPET_CAP) + '…'
        : (f.snippet ?? null),
    ...(f.hidden ? { hidden: true } : {}),
  }));
  let out = JSON.stringify(compact);
  if (out.length <= TOTAL_CAP) return out;
  // Tail-truncate to fit; very unlikely path.
  let lo = 0;
  let hi = compact.length;
  while (lo < hi) {
    const mid = (lo + hi + 1) >> 1;
    const candidate = JSON.stringify(compact.slice(0, mid));
    if (candidate.length <= TOTAL_CAP) lo = mid;
    else hi = mid - 1;
  }
  out = JSON.stringify(compact.slice(0, lo));
  return out;
}
...
export async function streamAgentReply(
  ...
  onEvent: (
    kind:
      ...
      // Message.generatedFiles), so a passive client can replace
      // its local state without reconciling deltas.
      | 'generated_files',
    data: string,
  ) => void,
  ...
  // events; dedupe on fileId (first occurrence wins) and drop the
